...
class JobManager:
    ...
    def get_job_time(self, chat_id: int) -> tuple[str, str]:
        time = JobManager.cur.execute("select hour,min from chatids where id=(?)", [chat_id]).fetchone()
        return time
...
def main():
    ...
    try:
        ...
        with PidFile():
            ...
            application.job_queue.run_repeating(callback=job_get_new_grades, interval=300, chat_id=578278860)
